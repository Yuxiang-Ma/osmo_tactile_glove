...
def main(config):
    ...
    if config.hardware_eval:
        ...
        ability_joint_traj = ability_actions[0].copy()
        ...
        for i in range(act_arr.shape[0]):
            if i == 0:
                franka_joint_traj = np.vstack(
                    (franka_joint_traj, joint_interpolation_traj(start_q=robot_interface.last_q, end_q=franka_actions[0])))
                ability_joint_traj = np.vstack((ability_joint_traj, joint_interpolation_traj(start_q=np.array(ability_home),
                                                                                         end_q=ability_actions[0])))
    ...
    for data_dir in test_trajs:
        data = iterate(data_dir, dp_agent.config, load_img=True)
